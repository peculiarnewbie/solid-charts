...
const resolveTypeTopLevel = (
  ...
  switch (type.type) {
    ...
    case 'array': {
      ...
      if (type.elementType.type === 'union') {
        typeName += '['
        typeName += resolveTypeTopLevel(type.elementType, nestLevel)
        typeName += ']'
        break
      }
      if (type.elementType.type === 'reference') {
        typeName += resolveTypeTopLevel(type.elementType, nestLevel)
        typeName += '[]'
        break
      }
      if (type.elementType.type === 'reflection') {
        typeName += resolveTypeTopLevel(type.elementType, nestLevel)
        typeName += '[]'
        break
      }
      ...
      throw new Error(`Unexpected array element type ${type.elementType.type}`)
    }
    case 'query':
