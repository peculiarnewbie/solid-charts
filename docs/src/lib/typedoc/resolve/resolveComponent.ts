...
const inheritAllowList: string[] = []
const noPropsComponentList: string[] = ['AxisCursor', 'AxisGrid', 'AxisLine']
...
const resolveComponent = (
  ...
  let props: PropType[] = []
  if (noPropsComponentList.includes(name)) {
    props = []
  } else {
    props = resolveComponentProps(componentDeclaration).sort((a, b) => {
  ...
  const dataTags = getTags('data', componentDeclaration.signatures![0].comment)
  const cssTags = getTags('css', componentDeclaration.signatures![0].comment)
...
const getReflectionProps = (type: ReflectionType) => {
  ...
  for (const prop of props as DeclarationVariant[]) {
    // Functions have signatures. Get the first signature and use its parameters.
    if (prop.signatures) {
